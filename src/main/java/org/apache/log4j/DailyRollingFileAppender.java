...
public class DailyRollingFileAppender extends FileAppender {
     ...
     * log
     *
     * 此时scheduledFilename = log.2020-02-03
     *
     */
    ...
    void rollOver() throws IOException {
        ...
        // 创建轮转文件名称的file，如果file已存在，则不要构建file
        ...
        // 并把文件名改为轮转的名字
        // 把当前写的log.log文件改为log.log.2020-02-02，log.log继续作为新文件给当前周期的文件输出流用
        File file = new File(fileName);
     ...
     *
     * 重写了输出方法，在输出时，增加了对文件时间周期的判断，如果周期变了，则重新构建文件进行输出
     *
     */
    protected void subAppend(LoggingEvent event) {
        long n = System.currentTimeMillis();
        // 不在一个时间点，则检查是否需要rolling文件
        if (n >= nextCheck) {
            now.setTime(n);
