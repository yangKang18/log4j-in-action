...
public class Category implements AppenderAttachable {
     ...
     *
     * 继承属性，每个logger都会指定parent，在输出日志时，先使用自身的所有appender输出日志，接下来继续使用parent的appender进行日志输出
     * 设置该字段为false，则表示不继承输出，也就是parent的appender不会打印，默认是继承输出。
     *
     */
    ...
    public void addAppender(Appender newAppender) {
        // 添加一个appender，appender的管理是有aai实例管理的
        if (aai == null) {
            aai = new AppenderAttachableImpl();
        }
        aai.addAppender(newAppender);
        // 每添加一个appender，都会触发一个添加事件
    ...
    public void callAppenders(LoggingEvent event) {
        ...
        for (Category c = this; c != null; c = c.parent) {
            ...
            synchronized (c) {
                ...
                // 如果当前logger的继承属性为false，则不在执行循环，也就是parent的appender不在输出
                if (!c.additive) {
                    break;
                }
            }
    ...
    synchronized void closeNestedAppenders() {
        ...
        if (enumeration != null) {
            while (enumeration.hasMoreElements()) {
                ...
                // 有些appender涉及IO操作，这些操作在appender生成时就存在了，在移除appender时，对他们进行关闭
    ...
    public void debug(Object message) {
        // 检查日志容器级别是否有能力处理debug级别的打印，没有则不输出
        if (repository.isDisabled(Level.DEBUG_INT)) {
            return;
        }

        // 在检擦debug级别是否大于等于该logger的级别，输出的行为只有大于日志级别时才会被输出
    ...
    protected void forcedLog(String fqcn, Priority level, Object message, Throwable t) {
        // 把日志器全限定名，日志对象，日志级别， 消息，异常包装为一个日志事件
    ...
    public Level getEffectiveLevel() {
        // 日志都是有parent和级别的，有的logger如果没有指定级别或者重置等原因，就使用夫logger的级别设置
        for (Category c = this; c != null; c = c.parent) {
            if (c.level != null) {
                return c.level;
            }
    ...
    public void removeAllAppenders() {
        if (aai != null) {
            ...
            // 移出aai中的所有appender后，把暂存的appender触发监听动作
     ...
     *
     * logger对象上的管理理论上应该只是关闭该对象上的所有appender即可，而不是关闭所有的，
     * 但是该静态方法最终是调用日志容器的shutdown方法，感觉不太合理
     *
     * @since 1.0
