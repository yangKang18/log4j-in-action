...
public class OptionConverter {
    ...
    static String findAndSubst(String key, Properties props) {
        ...
        try {
            /*
                log4j配置文件支持value中带有${}的值，例如：
                log.var1 = apple
                log.var2 = eat ${log.var1} ok?
                解析后log.var2的值为eat apple
     ...
     *
     *
     * 此方法用来解析配置文件中value中含有${}占位符的值，例如：
     * var1 = apple
     * var2 = eat ${var1} ok?
     * var2有占位符，占位符的变量为var1，解析式会把var1的value替换掉${var1}，最终var2 = eat apple ok? 这里支持多层嵌套，例如：
     * var3 = hello， ${var2}
     * 解析var3时，先替换为var2的value值，则为e${var1} ok?，此时会检查是否还有占位符，有则递归继续解析替换，最终var3 = hello，eat apple ok?
     *
    ...
    public static String substVars(String val, Properties props) throws
        ...
        while (true) {
            // 首先检查value中是有占位符${开头的，例如eat ${log.var1} ok?
            ...
            if (j == -1) {
                ...
                if (i == 0) { // this is a simple string
                    ...
                    // 表示解析过占位符的变量，对后面的内容直接追加进去，最后返回：eat apple ok?
                    sbuf.append(val.substring(i, val.length()));
                    return sbuf.toString();
                }
            } else {
                // buf先保存占位符${之前的字符串，即保存eat
                sbuf.append(val.substring(i, j));
                k = val.indexOf(DELIM_STOP, j);

                // 检查是否有}结尾的，如果没有结尾的，说明占位符不全，报错
                if (k == -1) {
                    ...
                    // 如果系统中没有该key对应的值的话，就从日志的配置文件中查值：apple
    ...
    public void selectAndConfigure(URL url, String clazz, LoggerRepository hierarchy) {
        ...
        // 加载文件如果是xml类型，则使用dom解析器
        if (clazz == null && filename != null && filename.endsWith(".xml")) {
            clazz = "org.apache.log4j.xml.DOMConfigurator";
        }

        // 如果有用户自定义解析类，则使用用户自定义的解析类，否则使用属性的配置解析器，也是默认的解析器
        if (clazz != null) {
            LogLog.debug("Preferred configurator class: " + clazz);
            configurator = (Configurator) instantiateByClassName(clazz,
                    Configurator.class,
        ...
        // 使用解析器解析配置文件，解析出来的对象放置到日志容器hierarchy中
