...
public class PatternParser {
    ...
    private void addToList(PatternConverter pc) {
        // 如果没有转化器，则首尾转化器指针指向该转化器
        if (head == null) {
            head = tail = pc;
        } else {
            // 如果已存在转化器，在尾部追加转换器，
            // 则把尾部指针指向转换器的next转换器指向新转换器，再把尾部指针指向新的转化器
            tail.next = pc;
            tail = pc;
        }
    }
                    ...
                    if (c == ESCAPE_CHAR) {
                        ...
                        switch (pattern.charAt(i)) {
                            ...
                            case 'n':
                                ...
                                break;
                            default:
                                // 如果缓存不为空，则说明缓存中是文字类型的字符，是不需要转化的，此时需要把缓存中的文字转变为一个特有的文字模板转化器保存下来
                                ...
                                // 缓存中的字符已保存在了文字模板转换器中，缓存中的字符可以清空了
                                currentLiteral.setLength(0);
                                // 能进入这里，一定是非%%，%n，如果此前没有文字则缓存添加当前%，并设置下一字符是转化字符，更改字符状态为转化器字符标志
